...
package doudou.library.com.refresh.custom;
...
import doudou.library.com.refresh.custom.util.RefreshViewHolder;

/**
 *
 * @author liubo
 * create at 16/7/11.下午3:23
 **/
public class CustomHeadViewRefreshViewHolder extends RefreshViewHolder {
    private CustomHeadViewFreshView mMeiTuanRefreshView;
     ...
     */
    public CustomHeadViewRefreshViewHolder(Context context, boolean isLoadingMoreEnabled) {
    ...
    public View getRefreshHeaderView() {
        if (mRefreshHeaderView == null) {
            mRefreshHeaderView = View.inflate(mContext, R.layout.view_refresh_header_custom, null);
            ...
            mMeiTuanRefreshView = (CustomHeadViewFreshView) mRefreshHeaderView.findViewById(R.id.mCustom_view);
            if (mPullDownImageResId != -1) {
                mMeiTuanRefreshView.setPullDownImageResource(mPullDownImageResId);
            } else {
                throw new RuntimeException("请调用" + CustomHeadViewRefreshViewHolder.class.getSimpleName() + "的setPullDownImageResource方法设置下拉过程中的图片资源");
            }
            if (mChangeToReleaseRefreshAnimResId != -1) {
                ...
                throw new RuntimeException("请调用" + CustomHeadViewRefreshViewHolder.class.getSimpleName() + "的setChangeToReleaseRefreshAnimResId方法设置进入释放刷新状态时的动画资源");
            }
            if (mRefreshingAnimResId != -1) {
                mMeiTuanRefreshView.setRefreshingAnimResId(mRefreshingAnimResId);
            } else {
                throw new RuntimeException("请调用" + CustomHeadViewRefreshViewHolder.class.getSimpleName() + "的setRefreshingAnimResId方法设置正在刷新时的动画资源");
            }
        }
